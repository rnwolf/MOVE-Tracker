...
def _create_excel_template(excel_path: str):
    """Creates a new Excel file with the required sheets and headers."""
    try:
        console.print(f"Creating Excel template at: [bold cyan]{excel_path}[/bold cyan]")
        ...
        ws_instructions['A3'] = "1. Populate 'Historic_Work_Items' with data from past projects to calculate baseline flow time."
        ws_instructions['A4'] = "2. Populate 'Current_Work_Items' with the work items for this project."
        ws_instructions['A5'] = "3. Populate 'MOVE_Configuration' with the project's parameters."
        ws_instructions['A6'] = "4. Do not delete items from 'Current_Work_Items'. Instead, mark them with a 'Date_Withdrawn'."
        ...
        config_data = [
            ...
            ("Planned_Delivery_Date", "2025-03-31"),
            ("Historic_50th_Percentile_Flow_Time_Override", ""),
            ("Buffer_Green_Date", "2025-04-15"),
            ("Buffer_Yellow_Date", "2025-04-30"),
            ("Buffer_Red_Date", "2025-05-15"),
        ...
        historic_headers = ["Historical_WI_ID", "Actual_Start_Date", "Actual_Completion_Date", "Flow_Time_Days"]
        ws_historic.append(historic_headers)

        # Current_Work_Items Sheet
        ws_current: Worksheet = wb.create_sheet("Current_Work_Items")
        current_headers = ["Work_Item_ID", "Description", "Status", "Actual_Start_Date", "Actual_Completion_Date", "Date_Withdrawn"]
        ws_current.append(current_headers)

        # Progress_Log Sheet
        ws_progress: Worksheet = wb.create_sheet("Progress_Log")
        progress_headers = [
            "Snapshot_Date", "Scope_At_Snapshot", "Actual_Work_Completed",
            "Elapsed_Time_Days", "Actual_Operational_Throughput", "Current_50th_Percentile_Flow_Time",
            "Forecasted_Delivery_Date", "Buffer_Consumption_Percentage", "Work_Done_Percentage",
        ...
        wb.save(excel_path)
        console.print(f"[bold green]Successfully created template: {excel_path}[/bold green]")

    except Exception as e:
        console.print(f"[bold red]An unexpected error occurred during template creation: {e}[/bold red]")
        logging.error(f"Template creation failed: {e}", exc_info=True)
        raise typer.Exit(code=1)


def _read_excel_data(excel_path: str) -> tuple[MOVEConfiguration, pd.DataFrame, pd.DataFrame]:
    ...
    try:
        with pd.ExcelFile(excel_path) as xls:
            if "MOVE_Configuration" not in xls.sheet_names:
                console.print("[bold red]Error: 'MOVE_Configuration' sheet not found in the Excel file.[/bold red]")
                raise typer.Exit(code=1)
            df_config = pd.read_excel(xls, "MOVE_Configuration").set_index("Parameter")["Value"]

            # --- Parse and validate MOVEConfiguration ---
            config_dict = {}
            required_params = [
                "Planned_Start_Date", "Planned_Delivery_Date", "Buffer_Green_Date",
                "Buffer_Yellow_Date", "Buffer_Red_Date", "Fever_Green_Yellow_Left_Y",
                "Fever_Green_Yellow_Right_Y", "Fever_Yellow_Red_Left_Y", "Fever_Yellow_Red_Right_Y"
            ]
            for param in required_params:
                if param not in df_config.index or pd.isna(df_config.get(param)):
                    console.print(f"[bold red]Error: Required configuration parameter '{param}' is missing or empty in 'MOVE_Configuration' sheet.[/bold red]")
                    raise typer.Exit(code=1)

            date_params = ["Planned_Start_Date", "Planned_Delivery_Date", "Buffer_Green_Date", "Buffer_Yellow_Date", "Buffer_Red_Date"]
            for param in date_params:
                config_dict[param.lower()] = pd.to_datetime(df_config[param]).date()

            float_params = [
                "Fever_Green_Yellow_Left_Y", "Fever_Green_Yellow_Right_Y",
                "Fever_Yellow_Red_Left_Y", "Fever_Yellow_Red_Right_Y"
            ]
            ...
            config_dict["historic_50th_percentile_flow_time_override"] = float(override_val) if pd.notna(override_val) else None
            ...
            sheet_to_df = {}
            required_sheets = {"Historic_Work_Items": ["Historical_WI_ID", "Actual_Start_Date", "Actual_Completion_Date"],
                               "Current_Work_Items": ["Work_Item_ID", "Status", "Actual_Start_Date", "Actual_Completion_Date"]}

            for sheet, required_cols in required_sheets.items():
                if sheet not in xls.sheet_names:
                    console.print(f"[bold red]Error: '{sheet}' sheet not found in the Excel file.[/bold red]")
                    raise typer.Exit(code=1)
                df = pd.read_excel(xls, sheet)
                if not all(col in df.columns for col in required_cols):
                     console.print(f"[bold red]Error: Missing one or more required columns in '{sheet}'. Required: {required_cols}[/bold red]")
                     raise typer.Exit(code=1)
            ...
            for col in ["Actual_Start_Date", "Actual_Completion_Date", "Date_Withdrawn"]:
                 if col in df_current.columns:
                    df_current[col] = pd.to_datetime(df_current[col], errors='coerce')


            console.print("[bold green]Successfully read and validated all input data.[/bold green]")
            return move_config, df_historic, df_current

    except FileNotFoundError:
        console.print(f"[bold red]Error: The file at '{excel_path}' was not found.[/bold red]")
        raise typer.Exit(code=1)
    except Exception as e:
        console.print(f"[bold red]An unexpected error occurred while reading the Excel file: {e}[/bold red]")
...
def _generate_full_progress_log(move_config: MOVEConfiguration, df_current: pd.DataFrame, historic_50th_percentile_flow_time: float, snapshot_date_str: str) -> pd.DataFrame:
    ...
    for current_ts in all_dates: # current_ts is a Timestamp
        ...
        buffer_consumption_percentage = max(0, buffer_consumption_percentage) # Cap at 0 if ahead
        ...
        log_entry['Current_50th_Percentile_Flow_Time'] = current_50th_percentile_flow_time
    ...
    if not progress_log_entries:
        console.print("[bold yellow]Warning: No progress log entries were generated.[/bold yellow]")
        return pd.DataFrame()
...
def _update_progress_log_sheet(excel_path: str, df_progress_log: pd.DataFrame):
    ...
    try:
        ...
        sheet = book.create_sheet("Progress_Log", -2) # Place it before the chart sheets
        ...
        console.print("[bold green]Successfully updated the 'Progress_Log' sheet.[/bold green]")

    except Exception as e:
        console.print(f"[bold red]An error occurred while updating the Progress_Log sheet: {e}[/bold red]")
        logging.error(f"Failed to update Excel sheet: {e}", exc_info=True)
        raise typer.Exit(code=1)

def _generate_charts(df_progress_log: pd.DataFrame, move_config: MOVEConfiguration, snapshot_date_str: str) -> Optional[tuple[str, str]]:
    ...
    # Plot data
    ax_we.plot(df_progress_log['Snapshot_Date'], df_progress_log['Actual_Work_Completed'], label='Actual Work Completed', marker='o', linestyle='-')
    ax_we.plot(df_progress_log['Snapshot_Date'], df_progress_log['Scope_At_Snapshot'], label='Scope At Snapshot', linestyle='--')

    # Trendline
    last_snapshot_data = df_progress_log.iloc[-1]
    x_numeric = (df_progress_log['Snapshot_Date'] - df_progress_log['Snapshot_Date'].min()).dt.days
    if len(x_numeric) > 1:
        m, b = np.polyfit(x_numeric, df_progress_log['Actual_Work_Completed'], 1)
        trendline_x = pd.date_range(start=df_progress_log['Snapshot_Date'].min(), periods=len(df_progress_log) + 90)
        trendline_y = m * (trendline_x - df_progress_log['Snapshot_Date'].min()).days + b
        ax_we.plot(trendline_x, trendline_y, 'r--', label='Completion Trend')

    # Extended Scope Line
    last_scope = last_snapshot_data['Scope_At_Snapshot']
    ax_we.axhline(y=last_scope, color='grey', linestyle='--', label=f'Current Scope ({last_scope})')

    # Vertical Buffer Lines
    ax_we.axvline(x=move_config.planned_delivery_date, color='blue', linestyle='--', label='Planned Delivery')
    ax_we.axvline(x=move_config.buffer_green_date, color='green', linestyle='--', label='Green Buffer')
    ax_we.axvline(x=move_config.buffer_yellow_date, color='yellow', linestyle='--', label='Yellow Buffer')
    ax_we.axvline(x=move_config.buffer_red_date, color='red', linestyle='--', label='Red Buffer')

    # Forecast Intersection
    forecasted_date = last_snapshot_data['Forecasted_Delivery_Date']
    if pd.notna(forecasted_date):
        ax_we.plot(forecasted_date, last_scope, 'X', color='purple', markersize=12, label=f'Forecast: {forecasted_date.strftime("%Y-%m-%d")}')
    ...
    # Background zones
    ax_fever.fill_between([0, 1], [move_config.fever_green_yellow_left_y, move_config.fever_green_yellow_right_y], 0, color='lightgreen', alpha=0.5, label='Green Zone')
    ax_fever.fill_between([0, 1], [move_config.fever_green_yellow_left_y, move_config.fever_green_yellow_right_y], [move_config.fever_yellow_red_left_y, move_config.fever_yellow_red_right_y], color='khaki', alpha=0.5, label='Yellow Zone')
    ax_fever.fill_between([0, 1], [move_config.fever_yellow_red_left_y, move_config.fever_yellow_red_right_y], 2, color='lightcoral', alpha=0.5, label='Red Zone') # y2=2 to fill up

    # Plot data
    ax_fever.plot(df_progress_log['Work_Done_Percentage'], df_progress_log['Buffer_Consumption_Percentage'], marker='o', linestyle='-', label='Project Path')

    # Highlight current point
    ax_fever.plot(last_snapshot_data['Work_Done_Percentage'], last_snapshot_data['Buffer_Consumption_Percentage'], '*', color='blue', markersize=15, label=f'Current ({snapshot_date_str})')

    ax_fever.set_xlabel("Work Done (%)")
    ax_fever.set_ylabel("Buffer Consumption (%)")
    ax_fever.set_xlim(0, 1)
    ax_fever.set_ylim(0, max(1.1, df_progress_log['Buffer_Consumption_Percentage'].max() * 1.2))
    ax_fever.legend()
    ax_fever.grid(True)
    plt.tight_layout()
...
def _insert_charts_into_excel(excel_path: str, chart_paths: tuple[str, str]):
    ...
    try:
        ...
        book.save(excel_path)
        console.print("[bold green]Successfully inserted charts into the Excel workbook.[/bold green]")

    except Exception as e:
        console.print(f"[bold red]An error occurred while inserting charts into Excel: {e}[/bold red]")
...
def main(
    ...
    create_template: bool = typer.Option(
        False, "--create-template", help="A boolean flag to create a new Excel file template."
    ),
    snapshot_date: Optional[str] = typer.Option(
        None, "--snapshot-date", help="Specifies the single date (YYYY-MM-DD) for processing."
    ),
    log_level: str = typer.Option(
        "INFO", "--log-level", help="Sets the logging verbosity.",
        case_sensitive=False,
    ),
    no_chart_insertion: bool = typer.Option(
        False, "--no-chart-insertion", help="If present, the script will process data but skip inserting charts."
    ),
    no_data_update: bool = typer.Option(
        False, "--no-data-update", help="If present, the script will generate charts but skip updating the Progress_Log sheet."
    ),
):
    """
    ...
    if create_template:
        ...
        _create_excel_template(output_path)
        raise typer.Exit()

    # Validate required arguments for processing
    if not excel_path or not snapshot_date:
        console.print("[bold red]Error: --excel-path and --snapshot-date are required unless --create-template is used.[/bold red]")
        raise typer.Exit(code=1)
    ...
    historic_50th_percentile_flow_time = _calculate_historic_flow_time(move_config, df_historic)

    # 3. Generate progress log
    df_progress_log = _generate_full_progress_log(move_config, df_current, historic_50th_percentile_flow_time, snapshot_date)
...
def _calculate_historic_flow_time(move_config: MOVEConfiguration, df_historic: pd.DataFrame) -> float:
    """Determines the historic 50th percentile flow time."""
    if move_config.historic_50th_percentile_flow_time_override is not None:
        logging.info(f"Using override for historic 50th percentile flow time: {move_config.historic_50th_percentile_flow_time_override}")
        return move_config.historic_50th_percentile_flow_time_override

    if df_historic.empty or 'Flow_Time_Days' not in df_historic.columns or df_historic['Flow_Time_Days'].isnull().all():
        logging.warning("Historic work items data is empty or lacks 'Flow_Time_Days'. Defaulting to 5 days.")
        return 5.0
    ...
    logging.info(f"Calculated historic 50th percentile flow time: {calculated_percentile:.2f} days")
